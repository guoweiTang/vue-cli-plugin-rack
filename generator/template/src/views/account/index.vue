 ...
 * @LastEditTime: 2021-10-13 16:34:27
-->
<template>
  <el-card>
    <el-alert
...
@Options({
  ...
  methods: mapActions('admin/user', ['setUserInfo']),
})
export default class extends Vue {
  ...
  // 图片上传
  async handleAvatarChange({ size, raw: file } : { size: number; raw: any}) {
  ...
  handleConfirmName(value: string) {
  ...
  handleConfirmSummary(value: string) {
    this.$store.dispatch('admin/user/setUserInfo', {
      summary: value,
    });
  }

  handleConfirmGender(value: string) {
    this.$store.dispatch('admin/user/setUserInfo', {
      gender: value,
    });
  }

  handleConfirmDescription(value: string) {
    this.$store.dispatch('admin/user/setUserInfo', {
      description: value,
    });
  }
